...
describe('persistent app session flow', () => {
  ...
  it('logout remains explicit after catalog release', async () => { const onLogout = jest.fn(); const snapshot = await visualAppAuthClient.status('visual-approved', 'device'); const screen = render(<AccessStatusScreen snapshot={snapshot} onLogout={onLogout} onRefresh={jest.fn()} onProfile={jest.fn()} onCatalog={jest.fn()} onVerifyPhone={jest.fn()} />); fireEvent.press(screen.getByLabelText('Sair da conta')); expect(onLogout).toHaveBeenCalledTimes(1); expect(screen.getByLabelText('Ver catálogo privado')).toBeTruthy(); });
