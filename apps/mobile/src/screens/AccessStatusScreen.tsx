...
function Action({ label, onPress, secondary = false, separated = false, disabled = false }: { label: string; onPress(): void; secondary?: boolean; separated?: boolean; disabled?: boolean }) {
  ...
  return (
    <Pressable
      ...
      onBlur={focus.onBlur}
      style={[s.button, secondary && s.secondary, s.statusAction, separated && s.statusActionSeparated, disabled && s.buttonMuted, focus.focusVisible && webFocusVisibleStyle]}
    >
...
export function AccessStatusHeader({ compact }: { compact: boolean }) {
  const s = useAuthStyles();
  return (
    <View accessibilityLabel="Cabeçalho do status de acesso" style={[s.statusHeader, compact && s.statusHeaderCompact]}>
      <Text style={[s.eyebrow, s.statusEyebrow]}>AEROSTORE · ACESSO</Text>
      <Text style={s.statusMeta}>ATUALIZADO AGORA</Text>
    </View>
  );
}
...
export function AccessStatusScreen({ snapshot, loading = false, error, onRefresh, onProfile, onCatalog, onLogout, onVerifyPhone }: { snapshot: AccessSnapshot; loading?: boolean; error?: string; onRefresh(): void; onProfile(): void; onCatalog(): void; onLogout(): void; onVerifyPhone(): void }) {
  ...
  const profileAvailable = snapshot.permissions.canViewProfile;
    ...
    <ScrollView style={s.page} contentContainerStyle={s.content}>
      <View style={s.card}>
        ...
        <View style={s.securityNote}>
          <Text style={s.securityNoteTitle}>Status verificado</Text>
          <Text style={s.securityNoteText}>
            {status === 'APPROVED' ? 'Seu catálogo é privado e protegido pela sua sessão AEROSTORE.' : 'Esta informação vem diretamente da AEROSTORE. O catálogo privado permanece protegido.'}
          </Text>
        </View>
        ...
        {loading ? <ActivityIndicator accessibilityLabel="Atualizando status" color={s.accent} style={s.inlineLoader} /> : null}
      </View>
    </ScrollView>
  );
}
