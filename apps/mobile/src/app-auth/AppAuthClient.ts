...
const visualSnapshot = (status: AccessStatus): AccessSnapshot => ({
  ...
  effectiveStatus: status, phoneVerified: status !== 'PENDING_PHONE_VERIFICATION', hasActiveMasterLink: status === 'APPROVED', requestStatus: status === 'PENDING_PHONE_VERIFICATION' ? 'PENDING_PHONE_VERIFICATION' : status === 'PENDING_APPROVAL' ? 'PENDING_APPROVAL' : status === 'REJECTED' ? 'REJECTED' : 'APPROVED', updatedAt: '2026-08-01T12:00:00.000Z', canViewCatalog: status === 'APPROVED', requiresAction: !['PENDING_APPROVAL', 'APPROVED'].includes(status), safeReasonCode: status === 'APPROVED' ? 'CATALOG_AVAILABLE' : 'VISUAL_SAFE_STATE', permissions: { canViewProfile: !['BLOCKED', 'CLOSED'].includes(status), canEditProfile: ['PENDING_APPROVAL', 'APPROVED'].includes(status), canViewCatalog: status === 'APPROVED' }
});
