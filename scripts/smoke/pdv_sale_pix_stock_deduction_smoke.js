...
/**
 * PDV Bloco C — Smoke 1: PIX payment + stock deduction + cancel (restores stock)
 *
 * Flow: login → open cash register → open session → add item → PIX payment → finalize → cancel (stock restored)
 * Confirmed: cancelSale via /api/pdv/sales/cancel/:saleId restores stock.
 */
...
blockProduction("pdv_sale_pix_stock_deduction_smoke.js");
warnLocalOnly("pdv_sale_pix_stock_deduction_smoke.js");
...
function log(msg) { console.log(`  [pix] ${msg}`); }
...
async function run() {
  console.log("\n=== Smoke C1: PIX payment + stock deduction + cancel ===\n");
  ...
  // 7. Verificar estoque após finalizar
  stockAfter = await getStock();
  log(`Estoque após finalizar: ${stockAfter}`);
  ...
  log(`Estoque após cancelar: ${stockFinal}`);
